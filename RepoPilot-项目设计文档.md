...
截至当前版本，RepoPilot 已经从“细粒度固定流水线”演进成**混合式 Agent runtime**：

- 外层使用轻状态机做治理
- `RETRIEVE` 阶段内部做动态检索和升级
- `PLAN` 输出结构化执行 contract
- `ACT` 按任务类型和 executor 路由
- `VERIFY / RECOVER` 保持显式控制
- 本地 Ollama `gemma4:26b` 已接入检索决策和 file-level code editing

因此，项目当前的重点已经不是“证明能不能跑通流程”，而是继续增强：

- 更强的 retrieval action set
- 更强的 executor 能力
- 更细的 approval / recovery / verification
...
- 外层状态机不再控制每一个微观动作
- `RETRIEVE` 变成阶段内 loop，而不是 `DECIDE_RETRIEVAL -> LOCAL_RETRIEVE -> MAP_REPO` 这样的硬编码链
- `PLAN` 决定执行器和编辑边界
- `RECOVER` 决定 retry / switch executor / rollback + replan

这意味着 RepoPilot 当前采用的是：

> 状态机做治理，阶段内部做 Agent 化决策
...
```json
{
  "task_type": "bug_fix | add_feature | add_test | refactor | doc_update | explain_repo | explain_target",
  "target": "解释 state_machine",
  "intent": "解释 state_machine",
  "target_symbols": ["state_machine"],
  "target_files": ["repopilot/app/state_machine.py"],
  "scope_hint": "file",
  "constraints": [
...
```json
{
  "retrieval_level": "LIGHT | LOCAL | GLOBAL",
  "search_targets": ["state_machine", "README.md"],
  "reason": "Task can start with targeted local retrieval before validation.",
  "decision_reasons": [
    "Task has a concrete target that supports local retrieval first."
  ],
...
```json
{
  "goal": "fix optional parameter in state_machine",
  "plan_kind": "code_edit",
  "requires_edit": true,
...
- 文档任务走 `builtin_doc`
- 补测试任务走 `builtin_test`
- 函数级代码任务优先 `builtin_code`
- file-level code task 默认走 `codex`
- `codex` 当前由本地 Ollama `gemma4:26b` 驱动
...
在失败时决定下一步动作，并保持失败控制流显式化。

**当前已实现恢复策略**

- retry same executor once
- switch from `codex` to `builtin_code` when a safe function contract exists
- rollback changed files and replan
- final rollback before fail when changes were already applied
